...
```sh
openssl dgst -sha384 -binary bootstrap-5.3.3.min.js | openssl base64 -A
```
...
```sh
npm install bootstrap-5.3.3@{{< param "current_version" >}}
```
...
```sh
yarn add bootstrap-5.3.3@{{< param "current_version" >}}
```
...
```sh
gem install bootstrap-5.3.3 -v {{< param current_ruby_version >}}
```
...
```sh
composer require twbs/bootstrap-5.3.3:{{< param current_version >}}
```
