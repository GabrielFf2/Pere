...
public class AsteroidJdbcDaoImpl implements AsteroidDao {
    ...
    public void save(Asteroid obj) {
        ...
        try {
            ...
            // Recuperem última ID insertada. Serà necessària per la foreign key dels aproach
            ResultSet rs = stmt.executeQuery("SELECT LAST_INSERT_ID()");
            int asteroidId = 0;
            if (rs.next()) {
                asteroidId = rs.getInt(1);
            }

            // Guardem totes les aproach
            if (obj.getAprochments() != null && !obj.getAprochments().isEmpty()) {
                for (Aproach aproach : obj.getAprochments()) {
                    String aproachQuery = "INSERT INTO aproach (aproachDate, velocity, distance, orbitingBody, id_asteroid) VALUES (" +
                            "'" + aproach.getAproachDate() + "', " +
                            aproach.getVelocity() + ", " +
                            aproach.getDistance() + ", " +
                            "'" + aproach.getOrbitingBody() + "', " +
                            asteroidId + ")";
                    stmt.execute(aproachQuery);
                }
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
